...
define( [ 'modernizr', 'js/widgets', 'jquery', 'js/plugins', 'js/extend' ], function( modernizr, widgets, $ ) {
    "use strict";

    var $form, model, repeatsPresent;
    ...
    function FormView( $form_, model_ ) {

        if ( typeof model_ == 'undefined' ) {
            return console.error( 'model needs to be defined (as instance of FormModel)' );
        }
        if ( typeof $form_ == 'undefined' ) {
            return console.error( 'jQuery form object needs to be defined' );
        }

        this.$ = $form_;
        $form = $form_;
        model = model_;
        repeatsPresent = ( $form.find( '.jr-repeat' ).length > 0 );
    ...
    FormView.prototype.outputUpdate = function( changedNodeNames ) {
        ...
        $form.find( ':not(:disabled) span.active' ).find( cleverSelector.join( ) ).each( function( ) {
            ...
            /*
                ...
                or the whole doc
            */
            $context = ( $( this ).parent( 'span' ).parent( 'label' ).find( '[name]' ).eq( 0 ).length === 1 ) ? $( this ).parent( ).parent( ).find( '[name]:eq(0)' ) : $( this ).parent( 'span' ).parent( 'legend' ).parent( 'fieldset' ).find( '[name]' ).eq( 0 ).length === 1 ? $( this ).parent( ).parent( ).parent( ).find( '[name]:eq(0)' ) : $( this ).closest( '[name]' );
    ...
    FormView.prototype.setEventHandlers = function( ) {
        ...
        */
        ...
        $form.on( 'change.file validate', 'input:not(.ignore), select:not(.ignore), textarea:not(.ignore)', function( event ) {
            ...
            console.log( 'change event with ', n );
